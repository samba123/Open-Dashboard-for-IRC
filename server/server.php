...
$topic = $phpobj->{'ResultSet'}->{'Result'}[0];
$topic='mysql';
...
function randNum ()
{
	...
	$fh = fopen("topic.txt", 'w') or die("can't open file");
	fwrite($fh, $result);
...
echo($top_id)."<br/><BR/>";
	...
	$op = file_get_contents("transcript.txt");
	...
	$fh = fopen("transcript.txt", 'w') or die("can't open file");
	fwrite($fh, $res);
