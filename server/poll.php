...
$newres="";
		...
		{
			$newres.="<a href=\"javascript:addATab(";
			$newres.= $k->get_qid()."_".$topic.")\">";
			$newres.= $k->get_ques()."</a><br/>";
	...
	else 	$res.='<root><questions>';
	$res.='</questions>';
	$new_result["questions"] = $newres;
	if ( isset($qid) )
	{
		...
		foreach($temp_arr as $k)
		{
			if($k->get_qid()==$qid)
			{
$new_result["transcript"] .= $k->get_ques()."<br/>".$k->get_ans();
	...
	$res.='<topic_list>';
$new_result["topic_list"] = array();
	foreach($Topics_Table as $key=>$val)
	{
		array_push($new_result["topic_list"],$key);
		$res.="<ti>".$key."</ti>";
	}
